...
export default function Checkout() {
  ...
  const [gateway, setGateway] = useState<"payu" | "cod">("cod");
  ...
  const payWithPayU = async () => {
    setLoading(true);
    try {
      const res = await fetch("https://backend.apnathikan.shop/api/orders/payu/initiate", {
        method: "POST", 
        headers: { 
  ...
  // ✅ Cash on Delivery (COD) फंक्शन
  const cashOnDelivery = async () => {
    setLoading(true);
    try {
      const res = await fetch("https://backend.apnathikan.shop/api/orders/create", {
        method: "POST",
        headers: { 
          "Content-Type": "application/json",
          "Authorization": user?.token ? `Bearer ${user.token}` : ""
